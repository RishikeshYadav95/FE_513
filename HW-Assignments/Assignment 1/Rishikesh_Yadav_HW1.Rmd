...
4. If element is lager than the mean, print ’True’, else print ’False’.
...
```{r}
log.return.plot <- ggplot(data = log.return, aes(x = log.return$AMZN.Adjusted)) + geom_histogram(color = "darkblue", fill = "lightblue", size = 1.2, bins = 100) + ggtitle("Daily Log Returns of the Amazon") + geom_vline(xintercept = quantile(x = as.vector(log.return), probs = 0.05), 
                                                                                                                                                  color = "red", size = 1.2, linetype = "longdash") + xlab("Total Return") + theme_minimal()
log.return.plot
```
